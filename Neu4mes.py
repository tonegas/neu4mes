...
class Neu4mec:
    def __init__(self, model_def = 0):
        ...
        #Keras structs
        self.relation_types = {
            'Linear': [self.setinput,self.createelem]
        }
    ...
    def createelem(self,relvalue,outel):
        for el in relvalue:
            if type(el) is tuple:
                if self.relations.get(el[0]) is None:
                    self.relations[el[0]+outel] = Dense(units = 1, activation = None, use_bias = None, name = "lin"+el[0]+outel)(self.inputs[el[0]])
                self.output_relation[outel].append(el[0])
                
            else: 
                if self.relations.get(el) is None:
                    self.relations[el+outel] = Dense(units = 1, activation = None, use_bias = None, name = "lin"+el+outel)(self.inputs[el])
                self.output_relation[outel].append(el)
                                    

    def setinput(self,relvalue):
        for el in relvalue:
            if type(el) is tuple:
    ...
    # class Linear():
    # def create_elem(self,relvalue,outel):
    #     for el in relvalue:
    #         if type(el) is tuple:
    #             if self.relations.get(el[0]) is None:
    #                 self.relations[el[0]+outel] = Dense(units = 1, activation = None, use_bias = None, name = "lin"+el[0]+outel)(self.inputs[el[0]])
...
#         #Keras structs
#         self.relation_types = {
#             'Linear': Linear() #[self.setinput,self.createelem] #Linear()
#         }
#         self.input_time_window = {}
#         self.input_n_samples = {}
#         self.max_n_samples = 0
